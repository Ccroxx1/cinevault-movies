...
export function updateDocumentSeo(options: SeoMetaOptions): void {
  ...
  const ogImage = options.ogImage || `${SITE_BASE_URL}/favicon.svg`;
  ...
  if (options.movie) {
    const m = options.movie;
    // Build Schema.org Movie structured data using strictly genuine API data without fabrication
    ...
    if (movieDesc && movieDesc.trim()) {
      movieJsonLd['description'] = movieDesc.trim();
    }
    ...
    // Only include aggregateRating if a genuine rating count exists from the API.
    // Strictly never use download_count or fake numbers like 1000.
    ...
    scriptEl.textContent = JSON.stringify(movieJsonLd);
  } else {
    // Default WebSite & Organization structured data for homepage
    const websiteJsonLd = {
      '@context': 'https://schema.org',
      '@type': 'WebSite',
      'name': SITE_NAME,
    ...
    scriptEl.textContent = JSON.stringify(websiteJsonLd);
  }
}
